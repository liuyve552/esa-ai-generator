...
- 缓存层级命中（内存/KV/实时生成）、TTL、延迟对比图（边缘 vs 中心模拟基线）
- “今日小任务（可打卡）”清单 + 进度
- 可传播动作：复制分享链接 / 系统分享 / 下载海报 / 朗读 / 收藏

3) 返回首页，切换模式（今日神谕/旅行助手/专注清单/情绪安抚/社交卡片），**不输入任何提示**直接点「生成」：输出明显不同。

4) 用**同一城市 + 同一模式 + 同一心情（同一天）**再生成一次：预期命中 **内存/KV 缓存**，端到端延迟明显降低。
...
浏览器(静态页面)
   │
   ├─(GET/POST) /api/generate?mode=...&lang=...&prompt=...&stream=1(可选)
   │        │
   │        ├─ 边缘读 Geo（优先 Header，缺失时 IP 接口兜底）
   │        ├─ 边缘拉 Weather（Open‑Meteo）
   │        ├─ 边缘生成内容（通义千问 / 无 Key 模板降级）
   │        ├─ 多级缓存：内存 Map → KV（86400s）→ 实时生成（每日自动刷新）
   │        ├─ 可选流式：边缘 Streaming 逐 token 返回（前端实时更新输出卡）
...
### 技术探索（在 81 的基础上再抬一档）
- **多级 KV 缓存可观测**：同一 mode+city+weatherCode+moodHash+dateSlice 命中内存/KV，体验接近 0ms。
...
5. KV（可选但推荐，用于“每日 KV 命中”与跨重启缓存）：
   - ESA：在控制台启用 EdgeKV，并创建 namespace `esa-ai-generator`（与 `edge/index.js` 的 `KV_NAMESPACE` 一致）。
   - Cloudflare：绑定 KV namespace 到 `env.GEN_KV`（代码会自动优先使用 `GEN_KV`，否则回退到 ESA `EdgeKV`/内存）。
6. 部署成功后，按「评委 1 分钟验收路径」验证即可。
